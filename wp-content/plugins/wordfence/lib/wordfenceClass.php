...
<IfModule !mod_authz_core.c>
	...
	public static function ajax_fixFPD_callback(){
		...
		if (wfScanEngine::testForFullPathDisclosure()) {
			...
			return array(
				...
				your php5.ini if your site is on a VPS or dedicated server that you control.",
			);
		}
		$issues->updateIssue($_POST['issueID'], 'delete');
		return array('ok' => 1);
	...
	private static function wfFunc_testmem(){
		...
		echo "This utility tests if your WordPress host respects the maximum memory configured\nin their php5.ini file, or if they are using other methods to limit your access to memory.\n\n--Starting test--\n";
		echo "Current maximum memory configured in php5.ini: " . ini_get('memory_limit') . "\n";
...
<pre class='wf-pre'>" . esc_textarea(self::getWAFBootstrapContent()) . "</pre>";
					...
					$wafActionContent .= "<p>{$additionally} will need to append the following code to your <code>php5.ini</code>:</p>
...
<p>We've also included instructions to manually perform the change if you are using a web server other than what is listed in the drop-down, or if file permissions prevent this change.</p>";
					...
					$wafActionContent .= "<p>You will need to remove the following code from your <code>php5.ini</code>, <code>.user.ini</code>, or <code>.htaccess</code> file:</p>
